...
(function () {
  ...
  // 北京时间（UTC+8）时钟。
  // 用「UTC 毫秒 + 8 小时」再读 UTC 字段来算，这样不受访客本地时区影响。
  function initClock() {
    var el = $("#clockCST");
    if (!el) return;
    function tick() {
  ...
  // 访问量：abacus 计数器（带 CORS，能取到 JSON）。
  // 每次加载自增一次，然后把总数渲染进页面；服务不可用就显示「—」，不影响其他功能。
  function initViewCounter() {
    var el = $("#viewCount");
    if (!el) return;
    var url = "https://abacus.jasoncameron.dev/hit/zava1on.github.io/homepage";
    var ctl = "AbortController" in window ? new AbortController() : null;
    ...
    fetch(url, { signal: ctl ? ctl.signal : undefined, cache: "no-store" })
      ...
      .then(function (d) {
        clearTimeout(timer);
        if (d && typeof d.value === "number") countUp(el, d.value);
        else el.textContent = "—";
      })
      .catch(function () { clearTimeout(timer); el.textContent = "—"; });
  }
